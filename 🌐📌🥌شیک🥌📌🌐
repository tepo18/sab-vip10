...
function abs(url, path) {
  try { return new URL(path, url).toString(); } catch { return path; }
}
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .logout{margin-left:auto;color:#ff8080;cursor:pointer;text-decoration:underline;font-size:12px}
</style>
</head>
<body>
<div class="container">
  <div class="header">
    ...
    <div class="logout" id="logout">خروج</div>
  </div>
  ...
  <div id="tab-sources" class="card" style="margin-top:10px">
    <h2>📡 Sources</h2>
    <div class="muted">Add/Remove your sub-links or raw config pages. Duplicates are ignored automatically.</div>
    <div style="margin-top:8px">
      <textarea id="addLinks" rows="5" placeholder="Paste vmess://, vless://, trojan://, ss:// or links (txt/json/sub-base64)"></textarea>
...
function maybeDecodeBase64AndExtract(text){
  const s = text.trim();
  if (!s) return [];
  // try base64 as whole content:
  const dec = b64decode(s);
  if (dec && /^(vmess|vless|trojan|ss):\/\//m.test(dec)) {
  ...
  async fetch(req, env, ctx) {
    try{
      ...
      if (path === "/login" && method === "GET") {
        if (authed) return Response.redirect(abs(req.url, "/panel"), 302);
      ...
      if (path === "/login" && method === "POST") {
        ...
        if (pass && pass === real) {
          return new Response("", {
            status:302,
            headers: {
              "Location": abs(req.url, "/panel"),
              "Set-Cookie": setCookie("auth","1", 3600*12)
            }
          });
        } else {
      ...
      if (path === "/logout" && method === "POST") {
        return new Response("OK", { headers: { "Set-Cookie": setCookie("auth","",0) }});
      }

      // Panel
      if (path === "/" || path === "/panel") {
        if (!authed) return Response.redirect(abs(req.url, "/login"), 302);
      ...
      if (path === "/api/settings") {
        ...
        if (method === "POST") {
          ...
          if (Array.isArray(js.protos)) {
            next.protos = js.protos.filter(p=>VALID_PROTOS.includes(p));
